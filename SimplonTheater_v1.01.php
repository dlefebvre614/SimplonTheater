 ...
 *																		*
 * Date du livrable : 30/04/2021                         Version : 1.01	*
 *																		*
 * Etude des limites pour ne pas que le client puisse commander plus de *
 * places que celles disponibles dans la rangée.						*
 * Traitement des erreurs de saisies.									*
 *																		*  
 * Date du livrable : __/__/____                         Version : 1.02	*
 *																		*
 * Ammélioration : 														*
 * 																		*
 * Vu le mode de fonctionnement d'un formulaire html, il vaut mieux 	*
 * proposer des choix valides de nombre de places et de rangées			*
 * libres dans les champs de saisie du formulaire, plutôt qu'une saisie *
 * qui est source d'erreur.	ainsi l'achat sera toujours valide.			*											*
 ************************************************************************/
...
$StVersion = "1.01"; /* current version */
$FileNameTranslation = array('\\' => '_', '/' => '_', ':' => '_', '*' => '_', '?' => '_', '<' => '_', '>' => '_', '|' => '_', "&" => "_","\"" => "_","'" => "_",";"=>"_","~"=>"-","+"=>""); /* translation characters for files names.*/
$debug = 1; /* si débug == 1 : affichage des variables en tête de page */
$sterr = 0; /* si sterr == 1 : affichage de l'erreur */
 ...
 ************************************************************************/
  ...
  * Récupération PHP des comptages par la fonction explode()				*
...
/* test du nombre de place libres restantes dans la rangée après achat */
if ($stnbli[$stline] < 0) {
	/* Erreur 1 : pas assez de place dans la rangée */
	$sterr = 1;
	$sterrtxt = 'Achetez '.-$stnbli[$stline].' place(s) sur un autre rang !';
	$stnbseat = 0;
	$stnbli[$stline] = 0;
	$stnboc[$stline] = $StNbSea;
}

/* test du numéro de rangée saisie par le client après achat */
if ($stline > $StNbRan) {
	/* Erreur 1 : le rang n'existe pas */
	$sterr = 1;
	$sterrtxt = 'le rang '.$stline.' n\'exsite pas !';
	$stline = $StNbRan;
}
...
/* saisie de l'achat en mode formulaire html, qui sera en method="post" en production pour ne pas de l'utilisateur agisse sur les variables get dans l'url*/
...
if ($sterr == 1) {
	echo '<td><font color='.$StOcCo.'>'.$sterrtxt.'</td>';
}
else {
...
echo '<p><a href="https://github.com/dlefebvre614/SimplonTheater.git">SimplonTheater.git</a> <font color='.$StCara.'>version '.$StVersion.' : traitement des erreurs de saisie</p></body></html>';
?>
